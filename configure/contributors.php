...
//Styles and scripts
function grd_contributor_admin_scripts() {
...
add_action( 'admin_enqueue_scripts', 'grd_contributor_admin_scripts' );

function grd_contributor_admin_styles() {
...
add_action( 'admin_enqueue_scripts', 'grd_contributor_admin_styles' );
...
add_action( 'welcome_panel', 'grd_custom_dashboard' );
function grd_custom_dashboard()
{
    ?>
    <h1>Hello!</h1>
    <?php
...
//remove settings
add_filter( 'screen_options_show_screen', 'grd_remove_screen_options' );
function grd_remove_screen_options() { 
...
//remove help
function grd_remove_help_tabs() {
  $screen = get_current_screen();
  $screen->remove_help_tabs();
}
add_action( 'admin_head', 'grd_remove_help_tabs' );
...
add_action('wp_dashboard_setup', 'grd_contributor_posts_widget');  
function grd_contributor_posts_widget() {
    global $wp_meta_boxes; 
    wp_add_dashboard_widget('contributor_posts_widget', 'Your Posts', 'grd_contributor_posts_loop');
} 
function grd_contributor_posts_loop() {
  global $current_user; wp_get_current_user();    
...
// REMOVE POST META BOXES AND MENUS
/*
add_action('admin_menu','remove_metaboxes_and_menus');
function remove_metaboxes_and_menus() {
  ...
  add_action( 'wp_before_admin_bar_render', 'artai_admin_bar_render' );
  function artai_admin_bar_render() {
    global $wp_admin_bar;
...
//Gutenberg settings
add_filter( 'allowed_block_types_all', 'grd_allowed_block_types', 10, 2 ); 
function grd_allowed_block_types( $allowed_blocks, $post ) { 
...
add_action( 'enqueue_block_editor_assets', 'grd_disable_fullscreen' );
function grd_disable_fullscreen() {
