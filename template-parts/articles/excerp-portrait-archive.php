<section>
<div class="ajax-container">



    <div class="row">

        <?php
        $category = get_queried_object();
        $category_id = $category->term_id;

        $counter = 0;

        $args_ex_p = array(
            'posts_per_page' => 6,
            'post_type' => 'post',
            'orderby' => 'date',
            'order'   => 'DESC',
            'category__in' => $category_id
            // 'suppress_filters' => true
        );
        $query_ex_p = new WP_Query( $args_ex_p );

        while ( $query_ex_p->have_posts() ) {
            $query_ex_p->the_post();
            $counter = $counter + 1;
            $auth_id = get_the_author_meta( 'ID' );
            //$auth_id_string = 'user_' . $auth_id;
        ?>    

        <div class="col-md-6 col-lg-4 excerp-portrait"  data-aos="fade-in" data-aos-duration="1200">
            <div class="excerp-portrait__wrapper"  onclick="window.location.href='<?php the_permalink(); ?>';">

                <div class="excerp-portrait__image article-image">

                    <div class="article-image__thumbnail">
                        <?php the_post_thumbnail( 'medium' ); ?>                                     
                    </div>

                </div>

                <div class="excerp-portrait__text-block">


                    <div class="article-info">
                        <div class="article-info__inner">
                            <div class="article-info__user">
                                <img class="article-info__user-image" src="<?php grd_get_author_avatar($auth_id); ?>"/>
                                <div class="article-info__user-name"><?php the_author(); ?></div>
                            </div>
                            <div class="article-info__date">
                                <div class="article-info__date-icon"></div>
                                <div class="article-info__date-text"><?php echo get_the_date(); ?></div>
                            </div>
                        </div>                   
                    </div> 

                    <div class="excerp-portrait__text">
                        <div class="excerp-portrait__category-button category-button"><?php the_category(' '); ?></div>
                        <h4 class="excerp-portrait__title" ><?php the_title(); ?></h4>
                        <div class="excerp-portrait__description"> <?php the_excerpt(); ?> </div>
                    </div>             

                </div>

            </div>  
        </div>

        <?php
            }   
            wp_reset_postdata();      
        ?>


    </div>


</div>




<div class="w-100 text-center" style="display:none;">
    <button class="btn btn-flat ajax-load-more" data-category="<?php echo $category_id; ?>" data-max="<?php echo get_category( $category_id )->count; ?>">Load more</button>
</div>




</section>


<script>


jQuery(document).ready( function( $ ){ 

    const ajaxurl = '<?php echo admin_url( "admin-ajax.php" ); ?>';
    let count = <?php echo $counter; ?>;      
    const category = $('.ajax-load-more').attr('data-category');
    const categoryMax = $('.ajax-load-more').attr('data-max');   

    if (count < categoryMax) {           
        $('.ajax-load-more').parent('div').fadeIn();
    } else { 
        $('.ajax-load-more').parent('div').fadeOut();
    }

    $('.ajax-load-more').click(function(){   

        const data = {
            action: 'loadPostsArchive',
            count: count,
            category: category
        };                

        $.ajax({
            type: "POST",
            data: data,
            url: ajaxurl,
            success: function(response) {
                if(response.success) {
                    $('.ajax-container').append(response.data.result);
                    //AOS.refresh()
                    setTimeout(() => {
                        $('.excerp-portrait').css('opacity', 1);
                    }, 50); 
                    count = response.data.newCount;
                    if (count >= categoryMax) {
                        $('.ajax-load-more').parent('div').fadeOut();
                    }
                };
            
            
            }

        });

        //return false;

    });
});
</script>